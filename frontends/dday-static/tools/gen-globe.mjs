...
/* ------------------------------------------------------------------ 대륙 윤곽
   ...
   단순화 문턱 0.2° — 처음에 0.5° 로 두었는데 휠 확대(최대 ×8)를 붙이면서 다시
   쟀다. 240px 지구본은 180° 를 240px 로 그리므로 ×1 에서 1px 이 0.75° 이지만
   ×8 에서는 0.094° 다. 0.5° 면 그 배율에서 5.3px 어긋나 눈에 보인다.
   좌표는 소수 첫째 자리로 굳힌다 — 0.1° 가 약 11km 이고 ×8 에서 1.1px 이라
   그보다 잘게 담아도 화면에 나오지 않는다. 실측한 두 지점:
     원본        링 128 · 꼭짓점 5,143
     0.5°        링 103 · 꼭짓점 1,634 · globe.json gzip  8.5KB · ×8 에서 5.3px
     0.2°  ←     링 126 · 꼭짓점 3,054 · globe.json gzip 13.7KB · ×8 에서 2.1px
   0.2° 를 고른 값은 5KB 다. 1400px 이상에서만 받는 자료이므로 그만큼은 쓴다 —
   확대해 놓고 해안선이 5px 어긋나 있으면 확대가 쓸모없어진다.
   ...
   1. **손으로 적은 육지·바다 검문 11개.** 서울은 육지, 남태평양은 바다처럼
      ...
      말하는 내용을 검사하는 것이다.
   2. **나라 점과의 대조 147/204.** 이미 검증한 점 자료를 이 링에 대 본다.
      섬나라·소국 57개는 110m 육지에 표현되지 않아 바다로 떨어지는 것이 맞다.
