...
├── gen-pages.mjs          data        → 한국어·영어 두 벌의 HTML (544개)
...
**"주말에 겹쳐 날아간 공휴일" 은 뺐다.** 재 보면 2026년 (국가, 날짜) 2,500쌍 중
540(21.6%)인데 그건 토·일 고정으로 센 값이라 이집트(금·토)에서 조용히 틀린다.
...
| 삭·보름 | 빈 원 · 찬 원 | 달이 안 보이는 날과 다 보이는 날 그대로 |
| 유성우 | 줄기 1·2·3개 | ZHR 100 이상 셋, 25 이상 둘, 나머지 하나 |
...
**색이 하나인 이유.** `<img>` 로 실으면 바깥 CSS 가 닿지 않는다 — `currentColor` 도
`base.css` 의 토큰도 그림 안으로 못 들어간다. SVG 안에 `prefers-color-scheme` 를 적는
길이 있지만 `<img>` 안에서의 동작이 브라우저마다 갈렸던 자리라, 두 테마에서 모두 읽히는
중간 잉크 하나(`#78849a`, 밝은 쪽 `--ink-3`)로 끝냈다. 흰 종이에서 3.6:1, 어두운
종이에서 5.1:1 이라 그림에 필요한 3:1 을 양쪽에서 넘는다.
...
생겼나 · 줄기 셋이 서로 겹치지 않나 — 를 본다.

마지막 항목은 **실제로 걸렸다.** 처음 앉힌 유성우 줄기 둘이 같은 선(`x+y=16`) 위에
있어서, 짧은 줄기가 아니라 첫째가 길게 이어진 것으로 보이고 그 중간에 머리 점이 하나
박혔다. 셋 다 기울기가 -1 이라 `x+y` 가 곧 그 선의 이름이고, 그 차이를 √2 로 나누면
두 줄기 사이 거리가 나온다.
...
`tools/check-pages.mjs` 가 544개 페이지를 브라우저 없이 돌려 본다.
...
20.5. **하늘 아이콘** — 원화 자체가 성립하나(상자를 넘지 않나 · 스물넷이 서로 다른
    자리인가 · 분점·지점이 동서남북인가 · 줄기 셋이 겹치지 않나), 파일이 원화와 1:1 이고
